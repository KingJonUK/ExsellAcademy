...
function pipelineCount(jobId: string): number {
  return employerPipeline.filter(
    (p) => p.employerId === demoEmployer.id && p.jobId === jobId,
  ).length;
}
...
export default function RolesPage() {
  ...
  const handleCreate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const job = createJob({
      ...
      minReadinessScore: get("minReadinessScore"),
    });
    if (job) {
      setVersion((v) => v + 1);
      form.reset();
      toast({
        title: "Role created",
        description: `${job.title} is now live in the talent pool.`,
      });
    }
  };
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
                ...
                <GlassCard key={job.id} className="shadow-soft">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge tone="neutral" className="whitespace-nowrap">
                      ...
                      {pipelineCount(job.id)} in pipeline
                    </Badge>
                  </div>
        ...
        <div>
          <GlassCard className="lg:sticky lg:top-8">
            ...
            <form ref={formRef} onSubmit={handleCreate} className="mt-5 flex flex-col gap-4">
