...
export default function CandidateProfilePage() {
  ...
  const learner = getCandidate(id);
  const jobs = employerJobs.filter((j) => j.status === "OPEN");

  const [existingStage, setExistingStage] = useState<PipelineStage | null>(
    () => findPipelineForLearner(id)?.stage ?? null,
  );
  const [selectedJobId, setSelectedJobId] = useState("");

  if (!learner || !learner.inTalentPool) {
    return (
      <div className="mx-auto max-w-5xl">
        ...
        <Card className="mt-6 flex flex-col items-center gap-2 py-12 text-center">
          ...
          <p className="text-sm font-semibold text-navy">Candidate not found</p>
          <p className="max-w-sm text-sm text-slate-500">
            This candidate is not in the talent pool or no longer available.
          </p>
        </Card>
      </div>
    );
  ...
  const handleRequest = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const entry = requestInterview(learner.id, selectedJobId || null);
    if (entry) {
      setExistingStage(entry.stage);
      toast({
        title: "Interview requested",
        description: `${name.split(" ")[0]} has been added to your pipeline.`,
      });
    }
  };
          ...
          <GlassCard className="lg:sticky lg:top-8">
            {existingStage ? (
              <>
                ...
                <div className="mt-4">
                  <PipelineStageBadge stage={existingStage} />
                </div>
                <Link
