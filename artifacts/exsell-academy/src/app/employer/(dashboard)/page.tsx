...
export default function EmployerDashboardPage() {
  ...
  const available = employerCandidates.filter((c) => c.inTalentPool).length;
  const interviews = employerPipeline.filter(
    (p) =>
      p.employerId === employer.id &&
      (p.stage === "INTERVIEW_REQUESTED" || p.stage === "INTERVIEW_BOOKED"),
  ).length;
  const inPipeline = employerPipeline.filter(
    (p) => p.employerId === employer.id && p.stage !== "REJECTED",
  ).length;
  const placements = employerPlacements.filter(
    (p) => p.employerId === employer.id,
  ).length;
  const recent = [...employerPipeline]
    .filter((p) => p.employerId === employer.id)
        ...
        <Card className="lg:col-span-2">
          ...
          {recent.length === 0 ? (
            ...
            <ul className="mt-4 divide-y divide-slate-100">
              {recent.map((entry) => {
                const learner = getCandidate(entry.learnerId);
                ...
                return (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      >
                        {learner?.user?.name ?? "Candidate"}
                      </Link>
                      <p className="truncate text-xs text-slate-500">
                        {job?.title ?? "No specific role"} ·{" "}
