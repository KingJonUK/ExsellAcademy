...
function isStatus(value: string | null): value is ApplicationStatus {
  return value !== null && STATUSES.includes(value as ApplicationStatus);
}
...
export default function Page() {
  ...
  const applications = getApplications(status);
              ...
              <tbody className="divide-y divide-slate-100">
                {applications.map((a) => (
                  <tr key={a.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-slate-600">
                      {getCourseRef(a.assignedCourseId)?.title ?? "—"}
                    </td>
                    <td className="px-6 py-4 text-slate-600">
                      {getSponsorRef(a.assignedSponsorId)?.organisation ?? "—"}
                    </td>
